...
import { activateSubscription, cancelSubscription, getSubscription, subscriptionState } from "../state/slices/subscription"
...
export const BlockSubscription: React.FC<Props> = ({ customerId }) => {
  ...
  return (
    <Block
      ...
      headerLeft={<Heading as="h3" fontSize="18px">Membership</Heading>}
    >
      ...
      <Box variant="rowStyle">
        <Flex justifyContent="flex-end">
          {subscription?.status && subscription?.status !== "canceled"
            && <Box><Button variant="smallghost" data-testid={dataTestIds.cancelButton} onClick={() => { dispatch(cancelSubscription(subscription.id)) }}>Cancel</Button></Box>}

          {!subscription?.status
            && <Box><Button variant="smallGreen" data-testid={dataTestIds.activateButton} onClick={() => { dispatch(showPopup({ "popup": "setup" })) }}>Upgrade</Button></Box>}

          {subscription?.status === "canceled" && hasCard
            && <Box><Button variant="small" data-testid={dataTestIds.activateButton} onClick={() => { dispatch(activateSubscription()) }}>Activate</Button></Box>}

          {subscription?.status === "canceled" && !hasCard
            && <Box><Button variant="small" data-testid={dataTestIds.activateButton} onClick={() => { dispatch(showPopup({popup: "upgrade"})) }}>Activate</Button></Box>}
        </Flex>
      </Box>

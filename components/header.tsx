...
export const Header: React.FC = () => {
  ...
  return (
    <Flex p="24px" fontSize={1} backgroundColor={colors.primary} justifyContent="space-around">
      <Flex width="100%" justifyContent="space-between">
        <Flex width={1 / 6}>
          <BiNetworkChart size="20px" />
          <Box variant="topMenuLink" ml="4px">
            ScrambledData
          </Box>
        </Flex>
        <Box width={4 / 6} maxWidth="1080px" data-testid={dataTestIds.container}>
          <NavigationTop />
        </Box>
        <Flex width={1 / 6} justifyContent="flex-end" pr="8px">
          <FaUserCircle
            data-testid={dataTestIds.accountIcon}
            color="white"
            cursor="pointer"
            size="20"
            onClick={(): void => {
